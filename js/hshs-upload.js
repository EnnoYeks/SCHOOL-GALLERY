(function () {
    ...
    function ensureStudioCss() {
        if (document.getElementById('hshsStudioCss')) return;
        var link = document.createElement('link');
        link.id = 'hshsStudioCss';
        link.rel = 'stylesheet';
        link.href = (function () {
            try {
                var s = document.querySelector('script[src*="hshs-upload"]');
                if (s && s.src) return s.src.replace(/js\/hshs-upload\.js.*$/, 'css/hshs-studio.css');
            } catch (e) {}
            return '/css/hshs-studio.css';
        })() + '?v=studio1';
    ...
    function render(errorMsg) {
        ...
        if (step === 'source') {
            ...
            html += '<div class="hshs-compose-card">';
            html += '<div class="hshs-preview hshs-preview-compact"><img id="hshsPreviewImg" src="' + escapeHtml(draft.src) + '" alt="preview" style="filter:' + filterCss(draft.filter) + '"></div>';
            html += '<textarea id="hshsCaption" rows="2" placeholder="What is happening on campus?">' + escapeHtml(draft.caption || '') + '</textarea>';
            html += '</div>';
            html += '<div class="hshs-section hshs-section-tight"><label>Class / house</label><div class="hshs-chips">';
            CLASS_TAGS.forEach(function (c) {
                html += '<button type="button" class="hshs-chip' + (draft.classTag === c ? ' on' : '') + '" data-class="' + c + '">' + c + '</button>';
            });
            html += '</div></div>';
            html += '<div class="hshs-section hshs-section-tight hshs-dest-section"><label>Pin on campus boards</label>';
            html += '<div class="hshs-dest-toolbar">';
            html += '<button type="button" class="hshs-dest-tool" data-dest-action="all">All</button>';
            ...
            DESTINATIONS.filter(function (d) { return d.kinds.indexOf(draft.kind) !== -1; }).forEach(function (d) {
                ...
                html += '<button type="button" class="hshs-dest-row' + (on ? ' on' : '') + '" data-dest="' + d.id + '" aria-pressed="' + (on ? 'true' : 'false') + '">';
            ...
            if (!draft.destinations.length) {
                html += '<p class="hshs-upload-error">Pick at least one board so classmates can find this.</p>';
            }
            html += '</div>';
            html += '<div class="hshs-section hshs-section-tight"><label>Filter</label><div class="hshs-filter-row" id="hshsFilterRow">';
            ...
            html += '</div></div>';
            html += '<div class="hshs-section hshs-section-tight"><label>Campus sound</label><div class="hshs-sound-row" id="hshsSoundRow">';
            SOUNDS.forEach(function (s) {
                html += '<button type="button" class="hshs-sound' + (draft.soundId === s.id ? ' on' : '') + '" data-sound="' + s.id + '" data-url="' + (s.url || '') + '"><b>' + escapeHtml(s.label) + '</b></button>';
            });
            html += '</div></div>';
            html += '<div class="hshs-compose-spacer"></div>';
            html += '</div>';
            html += '<div class="hshs-compose-bar">';
            html += '<button type="button" class="hshs-post-btn" id="hshsPublish"' + (!draft.destinations.length ? ' disabled' : '') + '>';
            html += 'Share to ' + draft.destinations.length + ' board' + (draft.destinations.length === 1 ? '' : 's');
            html += '</button></div>';
        }

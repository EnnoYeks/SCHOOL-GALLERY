...
(function () {
    ...
    function buildBar() {
        ...
        document.body.classList.add('has-mobile-shell', 'hshs-nav-slim', 'dark-mode');
        document.body.classList.remove('light-mode');
        const r = routes();
        const bar = document.createElement('nav'); bar.className = 'mobile-tabbar'; bar.setAttribute('aria-label', 'Primary');
        bar.innerHTML = `<a href="${r.PRIMARY[0].href}" class="${isActive(r.PRIMARY[0]) ? 'active' : ''}" data-tab="home"><i class="fas fa-house"></i><span>Home</span></a><a href="${r.PRIMARY[1].href}" class="${isActive(r.PRIMARY[1]) ? 'active' : ''}" data-tab="gallery"><i class="fas fa-images"></i><span>Gallery</span></a><button type="button" class="tab-upload" data-tab="upload" id="openUploadStudio" aria-label="Upload"><span class="tab-upload-btn"><i class="fas fa-plus"></i></span></button><a href="${r.PRIMARY[2].href}" class="${isActive(r.PRIMARY[2]) ? 'active' : ''}" data-tab="buzz"><i class="fas fa-bolt"></i><span>Buzz</span></a><a href="#more" class="" data-tab="more" id="openMoreSheet"><i class="fas fa-ellipsis"></i><span>More</span></a>`;

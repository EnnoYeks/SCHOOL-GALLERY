(function (g) {
  ...
  var PACKS = {
    ...
    people: { label: 'People', icon: '🙋', list: ['👋','🤚','🖐','✋','👌','✌️','🤞','👍','👎','👏','🙌','🙏','💪'] },
    love: { label: 'Love', icon: '❤️', list: ['❤️','🧡','💛','💚','💙','💜','💔','💕','💞','💗','💖'] },
  ...
  var mode='emoji', tab='smile';
  function loadCss(){ if(document.getElementById('hshs-chat-packs-css')) return; var l=document.createElement('link'); l.id='hshs-chat-packs-css'; l.rel='stylesheet'; l.href=(location.pathname.indexOf('/index/')!==-1?'../':'')+'css/hshs-chat-packs.css?v=260907pk1'; document.head.appendChild(l); }
  ...
  function render(){
    ...
    if(close) close.onclick=function(){ panel.hidden=true; panel.classList.remove('open'); };
    var abc=panel.querySelector('[data-dock="abc"]');
    if(abc) abc.onclick=function(){ if(close) close.click(); };
    var del=document.getElementById('hshsEmojiBackspace');
    if(del) del.onclick=function(){ var input=document.getElementById('hshsThreadInput'); if(input) input.value=input.value.slice(0,-1); };
  }
  ...
  function hijack(){ styleRec(); var btn=document.getElementById('hshsEmojiBtn'); if(!btn||btn.dataset.packsBound) return; btn.dataset.packsBound='1'; btn.addEventListener('click', function(){ setTimeout(render,0); }); }
  function boot(){ loadCss(); if(!document.getElementById('hshsChatPage')) return; hijack(); setTimeout(hijack,300); }

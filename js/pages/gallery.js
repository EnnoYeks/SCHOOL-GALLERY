/* Gallery — original look. Hydrate the HTML; do not replace it. */
export const pageId = 'gallery';
export const rootIds = ['galleryFeed', 'categoryFilter'];
export const bodyClass = 'light-mode';

export function render() {
  return `<div class="gallery-container" data-js-page="gallery">
...
export async function init(opts) {
  ...
  // gallery.js already boots itself when #galleryFeed exists.
  // Only construct it here if the HTML was injected by render() after a client route.
  if (!opts.hydrated && document.getElementById('galleryFeed') && !window.__hshsGalleryPage && window.GalleryPage) {
    window.__hshsGalleryPage = new window.GalleryPage();
  }
}

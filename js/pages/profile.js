(function (g) {
  ...
  function loadCss(){ if(document.querySelector('link[data-hshs-profile-css]')) return; var l=document.createElement('link'); l.rel='stylesheet'; l.href=base()+'css/hshs-profile.css?v=260912guest1'; l.setAttribute('data-hshs-profile-css','1'); document.head.appendChild(l); }

  /** Real signed-in user from store, or cloud profile from Firebase auth */
  function me(){
    try{
      // Prefer live Firebase / cloud profile when available
  ...
  function viewedUser(){
    ...
    var self = me();
    if (self) return self;
    // No account logged in → Guest User (not fake showcase data)
    return GUEST;
  }
  ...
  function userPosts(u){
    ...
    list = list.filter(function(p){ return mediaOf(p) || videoOf(p); });
    // Only use demo pack for a real logged-in user who has zero posts yet
    // Never show fake posts for Guest
  ...
  function paintTools(u, own){
    ...
    if (!box) return;
    if (isGuest(u)) {
      var loginHref = base() + 'index/login.html';
      box.innerHTML = '<a class="pf-btn" href="' + loginHref + '" style="text-decoration:none;display:inline-flex;align-items:center">Sign in</a>';
      return;
    }
    if (own) {
    ...
    box.innerHTML = '<button class="pf-btn" type="button" data-pf="follow">Follow</button><button class="pf-btn" type="button" data-pf="message">Message</button>';
  }
  ...
  function paintHighs(u){
    ...
    var items = [{add:true,label:'New'},{label:'Sports'},{label:'Events'},{label:'Class'},{label:'Studio'}];
    box.innerHTML = items.map(function(h){
      return '<button class="pf-hl" type="button"' + (h.add ? ' data-pf="create"' : '') + '><b>' + (h.add ? '+' : h.label.charAt(0)) + '</b><span>' + h.label + '</span></button>';
    }).join('');
  }
  ...
  function paintGrid(posts, u){
    ...
    if (!grid) return;
    if (isGuest(u)) {
      grid.innerHTML = '<div class="pf-empty">Sign in to see your posts and activity</div>';
      grid._posts = [];
      return;
    }
    ...
    grid.innerHTML = list.map(function(p,i){
      return '<button class="pf-cell" type="button" data-open="' + i + '"><img src="' + esc(mediaOf(p)) + '" alt="' + esc(p.title||'Post') + '" loading="lazy">' + (isVideo(p) ? '<i class="fas fa-play mark"></i>' : '') + '<span class="likes">' + abbr(p.likes||0) + '</span></button>';
    }).join('');
    grid._posts = list;
  }

  function openModal(post){
    var box = document.getElementById('pfModal'), stage = document.getElementById('pfModalStage'), cap = document.getElementById('pfModalCap');
    if (!box || !post) return;
    box.hidden = false;
  ...
  function paint(){
    ...
    if (isGuest(u)) {
      ...
      setTxt('pfBio', 'Sign in to view and edit your real profile.');
      setTxt('pfPosts', '0');
  ...
  function bind(){
    ...
    document.addEventListener('click', function(e){
      ...
      if (act === 'edit') {
        if (isGuest(u)) {
          location.href = base() + 'index/login.html';
          return;
        }
    ...
    // Re-paint when auth / profile changes (real account appears after sign-in)
    document.addEventListener('hshs:auth', function () { if (isPage()) paint(); });
    document.addEventListener('hshs:profile', function () { if (isPage()) paint(); });
  }
  ...
  g.HshsProfile = { mount: mount, paint: paint };

(function (global) {
  ...
  async function hydrate(elId) {
    ...
    try {
      ...
      if (!posts.length) { el.appendChild(global.HshsUI.emptyState('No photos yet.', 'fa-images')); return; }
      posts.forEach(function (p) {
        el.appendChild(global.HshsUI.postCard ? global.HshsUI.postCard(p) : global.HshsUI.mediaCard(p));
      });
    } catch (e) {
      global.HshsRender.clear(el);
  ...
  function mount() {
    ...
    global.HshsRender.mount(root, [
      ...
      global.HshsRender.el('div', { className: 'page-content', id: 'photosFeed', style: { padding: '1rem' } }, [global.HshsUI.skeleton(6)])
    ]);
    hydrate('photosFeed');

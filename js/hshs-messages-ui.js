(function (g) {
  ...
  function updateSendState() {
    ...
    var btn = $('hshsSendBtn');
    if (!btn) return;
    btn.classList.toggle('is-ready', !!(input && input.value.trim()));
  }
  ...
  function stopRecord(cancel) {
    ...
    if (recMedia && recMedia.state !== 'inactive') { try { recMedia.stop(); } catch (e) {} }
    if (recStream) { recStream.getTracks().forEach(function (t) { t.stop(); }); recStream = null; }
    var bar = $('hshsRecordBar');
    var compose = $('hshsComposeWrap');
    if (bar) bar.hidden = true;
    if (compose) compose.hidden = false;
    if (!cancel && recSecs > 0) appendMine('🎤 Voice note · ' + fmtTime(recSecs), { voice: true });
    recSecs = 0; recChunks = [];
  }
  ...
  function wire() {
    ...
    if (search && !search.dataset.bound) {
      ...
      search.addEventListener('input', function () { fillInbox(search.value); });
    }
    ...
    if (back && !back.dataset.bound) { back.dataset.bound = '1'; back.onclick = function () { showList(); }; }
    ...
    var input = $('hshsThreadInput');
    if (input && !input.dataset.bound) { input.dataset.bound = '1'; input.addEventListener('input', updateSendState); }
    ...
    if (emojiClose && !emojiClose.dataset.bound) { emojiClose.dataset.bound = '1'; emojiClose.onclick = closeEmoji; }
    ...
    var mic = $('hshsMicBtn');
    if (mic && !mic.dataset.bound) { mic.dataset.bound = '1'; mic.addEventListener('click', function () { startRecord(); }); }
    var recSend = $('hshsRecSend');
    if (recSend && !recSend.dataset.bound) { recSend.dataset.bound = '1'; recSend.onclick = function () { stopRecord(false); }; }
    ...
    if (recBar && !recBar.dataset.bound) {
      ...
      recBar.addEventListener('touchend', function (e) {
        if (e.changedTouches[0].clientX - startX < -60) stopRecord(true);
      }, { passive: true });
    }
